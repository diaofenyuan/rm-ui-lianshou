...
界面以白色为主，两个模式共用同一条主连接与同一份状态模型。背景说明与各面板的数据来源见[单兵模式与总控模式说明](docs/operator-mode.md)；连接池的 topic 分配、去重和实机验收边界见[多机器人连接池说明](docs/link-pool.md)。
...
兵种编号按官方协议附录二：1 号英雄、2 号工程、3/4/5 号步兵、6 号空中、7 号哨兵、8 号飞镖、9 号雷达；蓝方机器人 ID 在编号上加 100。MQTT 使用该机器人 ID。连接期间修改选择不会立即切换，点击“应用兵种并重连”后才生效；切换时清除旧比分和图传。选择兵种不会遥控官方选手端切换图传，也不表示所有操作位均已有专属功能。当前已接入全局比赛/态势域和按编号的单机域连接池：默认按所选阵营启动 1–7 号 7 条链路，总控模式可查看全队快照与链路健康度，单兵模式仍以所选主链路为准；机器人控制指令不在本客户端范围内。
...
- **单兵模式**：图传为主体 + 半透明 HUD。队友面板与全场地图不需要额外连接即可用（血量取自全局域 `GlobalUnitStatus`、点位取自 `RadarInfoToClient`）；连接池按编号保存单机域快照，总控模式通过我方全队列表与连接池状态面板展示 1–7 号链路，字段缺失时明确显示等待/过期状态（见[改造计划](docs/operator-mode-plan.md)批次 C/D）。
- MQTT 断开后约 2 秒（带 0–500ms 抖动）重连并重新订阅；同一 `(event_id,param)` 在 1.5 秒窗口内去重；超过 1.5 秒未收到信息标记过期，不自行猜测倒计时。图传中断时标明显示的是最后一帧。
...
| 多机器人连接池与按编号快照 | `src/robot_link_pool.cpp`、`src/match_state.cpp` |
| 总控全队与链路状态面板 | `src/ui/fleet_panel.cpp`、`src/ui/link_pool_panel.cpp` |
