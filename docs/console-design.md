...
连接池的 topic 分配、去重和实机验收边界见[多机器人连接池说明](link-pool.md)。
...
- **2.2.4 槽位顺序**：`robot_health` 按**己方** 1 / 2 / 3 / 4 / 7 号在前、**对方** 1 / 2 / 3 / 4 / 7 号在后，共 10 项（协议第 53 页数据编号 11）。**与 2.2.19 雷达的"对方在前"相反，两组槽位不可互相套用**；`robot_bullets` 只给己方。基地、前哨站与累计伤害字段同样自带"己方 / 对方"口径，界面按所选阵营映射到红方 / 蓝方。
